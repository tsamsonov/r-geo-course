...
Пакет [googlesheets](https://cran.r-project.org/web/packages/googlesheets/) разработан профессором статистики [Дженнифер Брайан](https://github.com/jennybc) для обеспечения онлайн-доступа к таблицам Google. Это очень удобный способ работы с табличными данными, который позволяет избавиться от манипуляций с файлами. Свои данные вы тоже можете хранить в таблицах Google. Единственное ограничение состоит в том, что для работы с такими данными вам нужен доступ в Интернет. С подробным руководством по использованию пакета вы можете ознакомиться [тут](https://cran.r-project.org/web/packages/googlesheets/vignettes/basic-usage.html). 
