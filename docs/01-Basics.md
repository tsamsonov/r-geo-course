...
# Типы данных, условия, ввод и ввывод {#data_types}
...
## [1]  0.91016002  0.54687145  0.07881036  1.16138244 -0.86076885
(b = 2 * a) # переменная a доступна, ее можно использовать и далее для вычислений
## [1]  1.8203200  1.0937429  0.1576207  2.3227649 -1.7215377
```
...
Например, можно проверить, является ли случайно сгенерированное число неотрицательным:
...
if (a < 0){
  ...
  cat('Получилось неотрицательное число!')
}
## Получилось неотрицательное число!
```
...
А мы оцениваем результат:

```r
if (is.numeric(phi)) { # проверяем, является ли введенное значение числом
...
1. Напишите выражение для вычисления расстояния между населенными пунктами, местоположение которых задано прямоугольными координатами, хранящимся в переменных `x1, y1, x2, y2`.
