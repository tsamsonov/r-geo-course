...
## [1]  0.5403670  2.5208026  0.4868229  1.7177913 -0.5386939
(b = 2 * a) # переменная a доступна, ее можно использовать и далее для вычислений
## [1]  1.0807340  5.0416051  0.9736458  3.4355827 -1.0773879
```
...
В процессе выполнения программы могут возникнуть ситуации, при которых дальнейшее выполнение программы невозможно. Например, пользователь вместо числа ввёл строковое значение. Хорошим тоном в данном случае будет не продолжить выполнение и ждать, пока программа сама споткнется и выдаст системное сообщение об ошибке, а обработать некорректный ввод сразу, сообщить об этом пользователю и остановить программу явным образом.

Для контроля за выполнением программы существует множество опций. Приведем несколько полезных функций:

- `stop(...)` выводит на экран объекты, перечисленные через запятую в `...` и завершает выполнение программы. При ручном вызове этой функции в `...` обычно передают текстовую строку с сообщением о причине остановки программы.
