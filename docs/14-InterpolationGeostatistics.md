...
Теоретическое значение $Z_0$, входящее в формулу ошибки, не известно, однако, как будет видно далее, его знание и не требуется, поскольку _ищется не сам квадрат отклонения, а его математическое ожидание_. Используя соотношение $\operatorname{Var}[X] = \operatorname{E}[X^2] - (\operatorname{E}[X])^2$, выразим среднюю квадратическую ошибку как:
...
Поскольку мы показали, что компонента $\operatorname{E}[Z^* - Z_0]$ при должном выборе коэффициентов может быть приравнена нулю, среднеквадратическую ошибку в этом случае можно вычислить через дисперсию:

$$\operatorname{E}[Z^* - Z_0]^2 = \operatorname{Var}[Z^* - Z_0]$$

Поскольку изначально мы имеем дело с распределением значений, а не их разностей, необходимо выразить дисперсию разностей в терминах статистических моментов исходной функции. Для этого воспользуемся следующими свойствами дисперсии и ковариации:
