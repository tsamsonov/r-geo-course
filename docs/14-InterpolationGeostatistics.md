...
- большинство природных явлений показывают зависимость наблюдаемых значений от их взаимного местоположения, которая проявляется в наличии корреляции значений $Z(\mathbf{p})$ и $Z(\mathbf{p} + \mathbf{h})$, где $\mathbf{h}$  вектор смещения между точками; чем короче $h$, тем, как правило, сильнее выражена корреляция значений

Упомянутая зависимость носит название __пространственной автокорреляции__. Наличие автокорреляции в географически распределенных данных отмечалось уже в трудах "отцов-основателей" математической статистики. Уильям Госсет (Стьюдент) в письме Карлу Пирсону (1900) заметил, что:
...
Британский статистик и биолог  __Рональд Эйлмер Фишер__ изучал пространственное распределение характеристик растений на опытных площадках, будучи сотрудником Ротамстедской агрохимической станции в 1920-1930-е гг. В своей книге «The Design of Experiments» (1935) он обращает внимание на то, что:

> _...всестороннее подтверждение получил тот факт, что близко расположенные участки более схожи, чем удаленные, судя по данным об урожайности...следовательно, наиболее верным будет делать [выборочные] блоки максимально компактными_

Для описания таких процессов необходимо ввести базовую математическую модель, а также определить ее свойства.

### Случайный процесс в пространстве и его моменты

Введение понятия случайного процесса позволяет сформировать базовую модель геостатистики. Пусть $p \in \mathbb{R}^k$ --- точка в $k$-мерном Евклидовом пространстве и $Z(p)$ --- __случайная величина__ в точке $p$. Тогда если $p$ меняется в пределах области $D \subset \mathbb{R}^k$ (эта область именуется _индексным множеством_), то формируется __случайный процесс__:

$$\{Z(p) | p \in D\}$$
Вертикальная черта в данной записи означает _условие_. То есть, переменная $p$ ограничена областью $D$. 
...
Случайный процесс, как и случайную величину, можно описать с помощью стандартных статистических моментов

__Математическое ожидание__ есть наиболее вероятная реализация случайного процесса:

$$E[Z(p)]=m(p)$$

Приведем пример, поясняющий смысл математического ожидания случайного процесса:
...
Геостатистическое оценивание производится аналогично простым линейным интерполяторам:

$$Z^*(p) = \sum_{p_i \in D(p)} w_i(p) Z(p_i),$$

за тем исключением что веса $w_i$ определяются путем анализа и моделирования пространственной автокорреляции.

> Эта процедура носит название __вариографии__.
...
где $m_i$  известные значения мат. ожиданий случайной функции в каждой точке, $m_0$  известное глобальное мат. ожидание случайной функции.
