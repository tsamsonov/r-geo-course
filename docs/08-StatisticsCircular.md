...
Создадим объект типа `circular` с направлениями ветра для анализа, и запишем его в новую переменую таблицы. Предварительно определим вспомогательную функцию, вычисляющую географический азимут на основе компонент скорости:
...
##  4 20674 Остров Ди…  2007     1     0  1700    29    35    47 0.6919214   
##  5 20674 Остров Ди…  2007     1     0  1600    28    38    49 0.6350267   
##  6 20674 Остров Ди…  2007     1     0  1500    26    40    50 0.5763752   
##  7 20674 Остров Ди…  2007     1     0  1400    25    41    51 0.5475622   
##  8 20674 Остров Ди…  2007     1     0  1300    25    42    54 0.5369107   
##  9 20674 Остров Ди…  2007     1     0  1200    24    45    56 0.4899573   
## 10 20674 Остров Ди…  2007     1     0  1100    24    49    58 0.4554511   
## # ... with 77,063 more rows
```

Выберем данные по высоте 0 метров за 12 часов дня для города Тикси, сохранив только составляющие скорости и ее скалярную величину:
...
Отобразим распределение направлений, розу-диаграмму и плотность распределения:
...
plot.circular(tiksi_wind$wind, 
     ...
     sub = 'Измерения за период с 2007 по 2018 г, высота 50 м')
...
Так же как и в случае с обычными данными, плотность распределения удобно использовать для определения модальных направлений, то есть наиболее часто встречающихся:
...
Проведем анализ направлений для всех станций. Для этого рассчитаем функции плотности распределения и разместим их в новом фрейме данных с лист-колонкой (да-да, в ячейках таблицы можно хранить сложные объекты!):
...
##    name            kden                     peak
##    <chr>           <list>                  <dbl>
##  1 Верхоянск       <S3: density.circular> -2.97 
##  2 Жиганск         <S3: density.circular> -3.20 
##  3 Кандалакша      <S3: density.circular> -0.347
##  4 Мирный          <S3: density.circular> -0.826
##  5 Мурманск        <S3: density.circular>  0.814
##  6 Нарьян-Мар      <S3: density.circular>  1.04 
##  7 Новолазаревская <S3: density.circular> -0.902
##  8 Норильск        <S3: density.circular> -0.877
##  9 Оленек          <S3: density.circular> -3.48 
## 10 Остров Диксон   <S3: density.circular> -0.145
## 11 Салехард        <S3: density.circular> -2.82 
## 12 Тикси           <S3: density.circular>  0.814
## 13 Шойна           <S3: density.circular>  0.561
```

После этого построим розы-диаграммы для всех станций:
