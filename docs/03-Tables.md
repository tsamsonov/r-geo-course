...
1. Решите предыдущую задачу с вычислением доли Федерального округа в населении на основе отдельной таблицы по населению России. Для этого создайте такую таблицу путем агрегирования данных по округам за каждый год. После этого _присоедините_ таблицу по стране к таблице по округам, используя год в качестве ключевого поля. Вычислите долю путем деления соответствующих полей с данными по населению.
    ...
    > __Подсказка:__ при создании таблицы с населением по России назовите поле с численностью населения именем, отличным от имени аналогичного поля в таблице округов. В этом случае вам будет проще не запутаться после присоединения. 
