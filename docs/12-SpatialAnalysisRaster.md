# Растровый анализ {#raster_analysis}
...
dist_grid = stations %>% 
  ...
  transmute(dist = st_as_sf(., as_points = TRUE, merge = FALSE) %>% 
           st_distance(stations, .) %>% 
           apply(2, min))

# Визуализируем результат
ggplot() +
  geom_stars(data = dist_grid) +
...
```r
temp_stars = getData("worldclim", var = "tmean", res = 10) %>% 
  st_as_stars() / 10
plot(temp_stars)
```

<img src="12-SpatialAnalysisRaster_files/figure-html/unnamed-chunk-9-1.png" width="384" />

```r


data(land, package = 'tmap')
types = land@data@attributes
...
land_stars = land %>% 
  ...
  mutate(cover = factor(cover, 
                        levels = types[[1]]$ID, 
                        labels = types[[1]]$cover),
         cover_cls = factor(cover,
                        levels = types[[2]]$ID, 
                        labels = types[[2]]$cover))
...
ggplot() +
  geom_stars(data = land_stars['cover']) +
  scale_fill_manual(values = pal, guide = guide_legend(ncol = 3), name = NULL) + 
  coord_sf(crs = st_crs(land_stars)) +
  theme(legend.position = 'bottom')
```

<img src="12-SpatialAnalysisRaster_files/figure-html/unnamed-chunk-9-2.png" width="384" />
...
## attribute(s):
##                cover                              cover_cls     
##  Water bodies     :393060   Forest                     :     0  
##  Snow / Ice       : 61986   Other natural vegetation   :     0  
##  Herbaceous       : 21377   Cropland                   :     0  
##  Tree Open        : 16171   Wetland                    :     0  
##  Sparse vegetation: 12247   Bare area/Sparse vegetation:     0  
##  Cropland         : 11658   (Other)                    :     0  
##  (Other)          : 66701   NA's                       :583200  
##      trees          elevation     
##  Min.   :  0.0    Min.   :-412    
...
## x    1 1080   -180  0.333333 +proj=longlat +datum=WGS8...    NA   NULL [x]
## y    1  540     90 -0.333333 +proj=longlat +datum=WGS8...    NA   NULL [y]

(cover_stars = st_warp(land_stars['cover'], temp_stars %>% slice(band, 1), method = 'near'))
...
```r
months = 1:12
cover_types = levels(cover_stars$cover)
zonal_stats = lapply(cover_types, function(X) {
  sapply(months, function(Y) {
    temp_month = temp_stars %>% slice(band, Y) %>% pull(1)
    cover_stars %>% 
      transmute(masked = ifelse(cover == X, temp_month, NA)) %>% 
      pull(masked) %>% 
      mean(na.rm = TRUE)
  })
}) %>% 
